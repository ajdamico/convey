...
Foster and all(1984) proposed a family of indicators to mesure poverty.
...
The argument `thresh_type` of this functions defines the type of poverty threshold adopted.
There are three possible choices:

1. `abs`-- fixed and given by the argument thresh_value
2. `relq`-- a proportion of a quantile, where the proportion is fixed the argument `proportion`and the quantile is defined by the argument `order`.
3. `relm`-- a proportion of the mean. the proportion is fixed the argument `proportion`

The quantile and the mean envolved in the definition of the threshold are estimmated for the whole population. When $\gamma=0$ and $\theta= .6*MED$ the measure is equal to the indicator àrpr` computed by
...
Consider first the poverty threshold fixed in the value $10000$. The headcount ratio is

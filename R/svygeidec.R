...
svygeidec.survey.design <-
  function (formula,
    ...
    for (i in seq_along(levels(grpvar))) {
      ...
      U_1 <-
        list(value = sum(incvar[w > 0] * w[w > 0]) , lin = incvar)
      list_all <-
        list(
          U_0_i = U_0_i,
          U_0 = U_0,
        ...
        grp.lin[, i] <-
          ifelse(
            ...
            U_fn(incvar , w_i , 1) ^ (-1) * incvar * log(incvar) -
              U_fn(incvar , w_i , 1) ^ (-1) * (T_fn(incvar , w_i , 1) * U_fn(incvar , w_i, 1) ^
                                                 (-1) + 1) * incvar +
              U_fn(incvar , w_i , 0) ^ (-1) ,
            0
          )
    ...
    for (i in seq_along(levels(grpvar))) {
      ...
      list_all <-
        list(
          ...
          p_i = p_i,
          gei_i = gei_i,
          epsilon = list(value = epsilon , lin = rep(0, length(incvar)))
        )
      ...
      if (epsilon == 0) {
        ...
        grp.est <-
          contrastinf(quote(g_i ^ epsilon * p_i ^ (1 - epsilon) * gei_i) , list_all)
        wtd.gei[i] <- grp.est$value
        wtd.gei.lin[, i] <- ifelse(w_i > 0 , grp.est$lin , 0)
        rm(grp.est)
...
svygeidec.svyrep.design <-
  function(formula,
    ...
    wtn.gei <-
      within.gei.fun(f.data = data.frame(incvar, grpvar, ws = ws))
    qq.wtn.gei <-
      apply(ww, 2, function(wi)
        within.gei.fun(f.data = data.frame(incvar, grpvar, ws =  wi)))
    ...
    qq.matrix <-
      matrix(
        c(qq.ttl.gei, qq.wtn.gei, qq.btw.gei),
        ncol = 3,
        dimnames = list(NULL, c("total", "within", "between"))
      )
    variance <-
      survey::svrVar(
        ...
        design$rscales,
        mse = design$mse,
        coef = matrix(ttl.gei, wtn.gei, btw.gei)
      )

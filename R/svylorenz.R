...
svylorenz.survey.design <-
  function (formula ,
    ...
    if (is.na(average)) {
      ...
      cis <-
        array(
          rbind(rep(NA, length(quantiles)), rep(NA, length(quantiles))),
          dim = c(2, length(quantiles)),
          dimnames = list(c("(lower", "upper)"), as.character(quantiles))
        )
      rval <-
        t(matrix(
          data = rep(NA, length(quantiles)),
          nrow = length(quantiles),
          dimnames = list(as.character(quantiles), as.character(formula)[2])
        ))
    ...
    L.p <-
      as.numeric(lapply(quantiles, function(x)
        wtd.psum(
          x = incvar, weights = w, q = x
        ))) / sum(w * incvar)
    ...
    if (empirical) {
      ...
      E_L.p <-
        cumsum(w[w != 0] * incvar[w != 0]) / sum(w[w != 0] * incvar[w != 0])
    ...
    var <- NULL
    lin_mat <-
      matrix(NA, nrow = length(ordincvar), ncol = length(quantiles))
    for (pc in quantiles) {
      i <- match(pc, quantiles)
      pc

      if (pc > 0 & pc < 1) {
        quant <- wtd.qtl(x = incvar,
                         q = pc,
                         weights = w)
        s.quant <- L.p[i]
        ...
        u_i <- 1 / design$prob
        u_i[u_i > 0] <-
          (1 / (N * average)) * (((incvar - quant) * (incvar <= quant)) + (pc * quant) - (incvar * s.quant))
...
svylorenz.svyrep.design <-
  function(formula ,
      ...
      function (x,
        ...
        res <-
          lapply(qs, function (q) {
            wtd.psum(x = x,
                     weights = weights,
                     q = q) / sum(weights[weights != 0] * x[weights != 0])
          })
        as.numeric(res)
      }
    ...
    L.p <-
      t(as.matrix(lapply_wtd.psum(
        x = incvar, qs = quantiles, weights = ws
      )))
    rval <-
      t(matrix(data = L.p, dimnames = list(as.character(quantiles))))
    qq <-
      apply(ww, 2, function(wi)
        lapply_wtd.psum(x = incvar, qs = quantiles, weights = wi))
    ...
    if (empirical) {
      ...
      ws <- ws[ordincvar]
      E_p <-
        (2 * cumsum(ws[ws != 0]) - ws[ws != 0]) / (2 * sum(ws[ws != 0]))
      E_L.p <-
        cumsum(ws[ws != 0] * incvar[ws != 0]) / sum(ws[ws != 0] * incvar[ws != 0])
    }

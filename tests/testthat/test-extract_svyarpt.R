...
a2 <- survey:::svyby(~eqIncome, by = ~db040, design = des_eusilc, FUN = svyarpt, order = 0.5, percent = 0.6,deff = FALSE)

b1 <- svyarpt(~eqIncome, design = des_eusilc_rep, 0.5, 0.6)

b2 <- survey:::svyby(~eqIncome, by = ~db040, design = des_eusilc_rep, FUN = svyarpt, order = 0.5, percent = 0.6,deff = FALSE)

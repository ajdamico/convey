...
for ( this.g in c(0,1,2) )  {
  ...
  test_that( paste0( "svyfgt g=", this.g , " works on unweighted designs"), {
    expect_false( is.na ( coef( svyfgt( ~api00, design=dstrat1 , g = this.g , abs_thresh = 7000 , type_thresh = "abs" ) ) ) )
    expect_false( is.na ( SE( svyfgt( ~api00, design=dstrat1 , g = this.g , abs_thresh = 7000 , type_thresh = "abs" ) ) ) )
  } )

...
test_that("svyzengacurve works on unweighted designs",{
	svyzengacurve(~api00, design=dstrat1)
})
...
a1 <- svyzengacurve(~eqincome, design = des_eusilc )
a2 <- svyby(~eqincome, by = ~db040, design = des_eusilc, FUN = svyzengacurve )

b1 <- svyzengacurve(~eqincome, design = des_eusilc_rep )

b2 <- svyby(~eqincome, by = ~db040, design = des_eusilc_rep, FUN = svyzengacurve )

SE_dif1 <- max(abs(SE(a1)-SE(b1)))
SE_diff2 <- max(abs(SE(a2)-SE(b2)))

test_that("output svyzengacurve",{
  ...
  expect_lte(SE_dif1, max( coef(a1) ) * 0.05 ) # the difference between CVs should be less than 5% of the coefficient, otherwise manually set it
  ...
  expect_lte( max(confint(a1)[,1] - coef(a1)), 0)
  expect_gte( min(confint(a1)[,2] - coef(a1)), 0)
  expect_lte( max(confint(b1)[,1] - coef(b1)), 0)
  expect_gte( min(confint(b1)[,2] - coef(b1)), 0)
  expect_equal(sum(confint(a2)[,1]<= coef(a2)),length(coef(a2)))
  expect_equal(sum(confint(a2)[,2]>= coef(a2)),length(coef(a2)))
  expect_equal(sum(confint(b2)[,1]<= coef(b2)),length(coef(b2)))
  expect_equal(sum(confint(b2)[,2]>= coef(b2)),length(coef(b2)))
  expect_lte( max( (SE(a1)/coef(a1))[-1] ), .05)
  expect_lte( max( (SE(b1)/coef(b1))[-1] ), .05)
})
	...
	c1 <- svyzengacurve( ~ eqincome , design = dbd_eusilc )
...
sub_des <- svyzengacurve( ~eqincome , design = subset( des_eusilc , db040 == "Burgenland") )
sby_des <- svyby( ~eqincome, by = ~db040, design = des_eusilc, FUN = svyzengacurve)
sub_rep <- svyzengacurve( ~eqincome , design = subset( des_eusilc_rep , db040 == "Burgenland") )
sby_rep <- svyby( ~eqincome, by = ~db040, design = des_eusilc_rep, FUN = svyzengacurve)
...
# second run of database-backed designs #
	...
	sub_dbd <- svyzengacurve( ~eqincome , design = subset( dbd_eusilc , db040 == "Burgenland") )
	sby_dbd <- svyby( ~eqincome, by = ~db040, design = dbd_eusilc, FUN = svyzengacurve)
	sub_dbr <- svyzengacurve( ~eqincome , design = subset( dbd_eusilc_rep , db040 == "Burgenland") )

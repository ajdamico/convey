context("svygei output survey.design and svyrep.design")
...
a2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc, FUN = svygei )

b1 <- svygei(~eqincome, design = des_eusilc_rep )

b2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svygei )
...
test_that("output svygei",{
  ...
  expect_equal(sum(confint(b2)[,2]>= coef(b2)),length(coef(b2)))
	...
	c2 <- svyby(~ eqincome, by = ~hsize, design = dbd_eusilc, FUN = svygei )
...
sub_des <- svygei( ~eqincome , design = subset( des_eusilc , hsize == 1) )
sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svygei)
sub_rep <- svygei( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) )
sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svygei)
...
# second run of database-backed designs #
	...
	sub_dbd <- svygei( ~eqincome , design = subset( dbd_eusilc , hsize == 1) )
	sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc, FUN = svygei)
	sub_dbr <- svygei( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) )
	sby_dbr <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc_rep, FUN = svygei)

...
test_that("svyamato works on unweighted designs",{
  svyamato(~api00, design=dstrat1)
})


data(eusilc) ; names( eusilc ) <- tolower( names( eusilc ) )
eusilc$rb090 <- factor( eusilc$rb090, ordered = T )

des_eusilc <- svydesign(ids = ~rb030, strata =~db040,  weights = ~rb050, data = eusilc)
des_eusilc <- convey_prep(des_eusilc)
des_eusilc_rep <-as.svrepdesign(des_eusilc, type= "bootstrap")
des_eusilc_rep <- convey_prep(des_eusilc_rep)
a1 <- svyamato(~eqincome, design = des_eusilc )
a2 <- svyby(~eqincome, by = ~rb090, design = des_eusilc, FUN = svyamato )
...
if( .Machine$sizeof.pointer > 4 ){
  ...
  c1 <- svyamato( ~ eqincome , design = dbd_eusilc )
...
sub_des <- svyamato( ~eqincome , design = subset( des_eusilc , rb090 == "male" ) )
sby_des <- svyby( ~eqincome, by = ~rb090, design = des_eusilc, FUN = svyamato )
sub_rep <- svyamato( ~eqincome , design = subset( des_eusilc_rep , rb090 == "male" ) )
sby_rep <- svyby( ~eqincome, by = ~rb090, design = des_eusilc_rep, FUN = svyamato )
...
if( .Machine$sizeof.pointer > 4 ){
  ...
  sub_dbd <- svyamato( ~eqincome , design = subset( dbd_eusilc , rb090 == "male" ) )
  sby_dbd <- svyby( ~eqincome, by = ~rb090, design = dbd_eusilc, FUN = svyamato )
  sub_dbr <- svyamato( ~eqincome , design = subset( dbd_eusilc_rep , rb090 == "male" ) )

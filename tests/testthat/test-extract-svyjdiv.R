...
# return test context
context("jdiv output survey.design and svyrep.design")
...
test_that("svyjdiv works on unweighted designs", {
  expect_false(is.na (coef(
    svyjdiv(~ api00, design = dstrat1 , epsilon = this.epsilon)
  )))
...
test_that("error on income <= 0 " , expect_error(svyjdiv(~ eqincome , des_eusilc , epsilon = this.epsilon)))
...
# calculate estimates
a1 <-
  svyjdiv(~ eqincome , des_eusilc , deff = TRUE , linearized = TRUE, influence = TRUE )
a2 <-
  svyby(~ eqincome ,
        ~ hsize,
        des_eusilc,
...
test_that("output svyjdiv" , {
  ...
  # compare with svygei
  expect_equal(as.numeric(coef(a1)) , as.numeric(sum(sapply(list(
    d1 , e1
  ) , coef))))
  expect_equal(as.numeric(attr(a1 , "linearized")) , as.numeric(rowSums(sapply(
    list(d1 , e1) , attr , "linearized"
  ))))
  expect_equal(as.numeric(attr(a1 , "influence")) , as.numeric(rowSums(sapply(
    list(d1 , e1) , attr , "influence"
  ))))
  expect_equal(attr(a1 , "index") , attr(d1 , "index"))
  expect_equal(attr(a1 , "index") , attr(e1 , "index"))

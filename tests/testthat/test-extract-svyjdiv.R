...
test_that("svyjdiv works on unweighted designs", {
  expect_false( is.na ( coef( svyjdiv( ~api00, design=dstrat1 ) ) ) )
  expect_false( is.na ( SE( svyjdiv( ~api00, design=dstrat1 ) ) ) )
} )
...
test_that( "error on income <= 0 " , expect_error( svyjdiv( ~eqincome , des_eusilc )  ) )

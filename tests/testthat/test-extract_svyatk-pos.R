...
a1 <- svyatk(~eqincome, design = des_eusilc )
a2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc, FUN = svyatk )

b1 <- svyatk(~eqincome, design = des_eusilc_rep )

b2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyatk )
...
test_that("output svyatk",{
  ...
  expect_equal(sum(confint(b2)[,2]>= coef(b2)),length(coef(b2)))
	...
	c1 <- svyatk( ~ eqincome , design = dbd_eusilc )
...
sub_des <- svyatk( ~eqincome , design = subset( des_eusilc , hsize == 1) )
sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyatk)
sub_rep <- svyatk( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) )
sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyatk)
...
# second run of database-backed designs #
	...
	sub_dbd <- svyatk( ~eqincome , design = subset( dbd_eusilc , hsize == 1) )
	sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc, FUN = svyatk)
	sub_dbr <- svyatk( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) )

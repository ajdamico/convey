  ...
  for (this_dimw in list( NULL, c(.25, .75) )) {
    for ( this_k in c( .5 , 1 ) ){
      for ( this_g in c( 0 , 1 , 2 ) ) {
        svyafc( ~api00+pcttest, design=dstrat1, cutoffs = list( 400, 90 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )
      }
    }
  }
})
  ...
  for (this_dimw in list( NULL, c(.25, .75) )) {
    for ( this_k in seq( 1/3, 1, 1/3 ) ){
      for ( this_g in c( 0 , 1 ) ) {
        ...
        a1 <- svyafc( ~eqincome+hy050n, design=des_eusilc, cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )

        a2 <- svyby( ~eqincome+hy050n, by = ~rb090, design = des_eusilc, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)

        b1 <- svyafc( ~eqincome+hy050n, design=des_eusilc_rep, cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )

        b2 <- svyby( ~eqincome+hy050n, by = ~rb090, design = des_eusilc_rep, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)
        ...
        # database-backed design
        c1 <- svyafc( ~eqincome+hy050n, design=dbd_eusilc, cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )

        c2 <- svyby( ~eqincome+hy050n, by = ~rb090, design = dbd_eusilc, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)
        ...
        sub_des <- svyafc( ~eqincome+hy050n, design=subset( des_eusilc, rb090 == "male" ), cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )
        sby_des <- svyby( ~eqincome+hy050n, by = ~rb090, design = des_eusilc, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)
        ...
        # create a hacky database-backed svrepdesign object
        # mirroring des_eusilc_rep
        sub_dbd <- svyafc( ~eqincome+hy050n, design=subset( dbd_eusilc, rb090 == "male" ), cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )
        sby_dbd <- svyby( ~eqincome+hy050n, by = ~rb090, design = dbd_eusilc, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)
        sub_dbr <- svyafc( ~eqincome+hy050n, design=subset( dbd_eusilc_rep, rb090 == "male" ), cutoffs = list( 7000, 3000 ), g = this_g, k = this_k, dimw = this_dimw, na.rm = FALSE )
        sby_dbr <- svyby( ~eqincome+hy050n, by = ~rb090, design = dbd_eusilc_rep, FUN = svyafc, g=this_g, k= this_k, dimw = this_dimw, cutoffs = list( 7000, 3000 ), deff = FALSE)

...
sub_rep <- svyarpr( ~eqincome , design = subset( des_eusilc_rep , db040 == "Burgenland" ) )
sby_rep <- svyby( ~eqincome, by = ~db040, design = des_eusilc_rep, FUN = svyarpr)
...
if( .Machine$sizeof.pointer > 4 ){
	...
	sub_dbd <- svyarpr( ~eqincome , design = subset( dbd_eusilc , db040 == "Burgenland" ) )
	sby_dbd <- svyby( ~eqincome, by = ~db040, design = dbd_eusilc, FUN = svyarpr)
	sub_dbr <- svyarpr( ~eqincome , design = subset( dbd_eusilc_rep , db040 == "Burgenland" ) )
	sby_dbr <- svyby( ~eqincome, by = ~db040, design = dbd_eusilc_rep, FUN = svyarpr)

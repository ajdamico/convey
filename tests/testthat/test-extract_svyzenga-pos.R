...
# a2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc, FUN = svyzenga )

# b1 <- svyzenga(~eqincome, design = des_eusilc_rep )

# b2 <- svyby(~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyzenga )
...
# if( .Machine$sizeof.pointer > 4 ){
	...
	# c2 <- svyby(~ eqincome, by = ~hsize, design = dbd_eusilc, FUN = svyzenga )
...
# sub_des <- svyzenga( ~eqincome , design = subset( des_eusilc , hsize == 1) )
# sby_des <- svyby( ~eqincome, by = ~hsize, design = des_eusilc, FUN = svyzenga)
# sub_rep <- svyzenga( ~eqincome , design = subset( des_eusilc_rep , hsize == 1) )
# sby_rep <- svyby( ~eqincome, by = ~hsize, design = des_eusilc_rep, FUN = svyzenga)
...
# if( .Machine$sizeof.pointer > 4 ){
	...
	# sub_dbd <- svyzenga( ~eqincome , design = subset( dbd_eusilc , hsize == 1) )
	# sby_dbd <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc, FUN = svyzenga)
	# sub_dbr <- svyzenga( ~eqincome , design = subset( dbd_eusilc_rep , hsize == 1) )
	# sby_dbr <- svyby( ~eqincome, by = ~hsize, design = dbd_eusilc_rep, FUN = svyzenga)

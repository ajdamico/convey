...
for ( this.epsilon in c(0,.5,1,2) ) {
  ...
  # return test context
  context( paste( "gei epsilon =" , this.epsilon , "output survey.design and svyrep.design" ) )
  ...
  test_that("svygei works on unweighted designs", {
    expect_false( is.na ( coef( svygei( ~api00, design=dstrat1 , epsilon = this.epsilon ) ) ) )
    expect_false( is.na ( SE( svygei( ~api00, design=dstrat1 , epsilon = this.epsilon ) ) ) )
  } )

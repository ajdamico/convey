...
# S3 generic function
survey_qsr <- function(
  x, na.rm = FALSE, vartype = c("se", "ci", "var", "cv"), alpha1 = 0.20,
  alpha2= (1-alpha1), upper_quant = FALSE, lower_quant = FALSE, upper_tot = FALSE,
  .svy = current_svy(), ...
) {
  UseMethod("survey_qsr", .svy)
}

## ------------------------------------------------------------------------
survey_qsr.tbl_svy <- function(
  x, na.rm = FALSE, vartype = c("se", "ci", "var", "cv"), alpha1 = 0.20,
  alpha2= (1-alpha1), upper_quant = FALSE, lower_quant = FALSE, upper_tot = FALSE,
  .svy = current_svy(), ...
) {
  ...
  .svy <- srvyr::set_survey_vars(.svy, x)
  attributes(.svy)$full_design <- srvyr::set_survey_vars(attributes(.svy)$full_design,x)
  out <- convey::svyqsr(~`__SRVYR_TEMP_VAR__` , alpha1 = alpha1, alpha2 = alpha2, upper_quant = FALSE,
  lower_quant = FALSE, upper_tot = FALSE, na.rm = na.rm, design = .svy)
  out <- srvyr::get_var_est(out, vartype)
  out
}
...
survey_qsr.grouped_svy <- function(
  x, na.rm = FALSE, vartype = c("se", "ci", "var", "cv"), alpha1 = 0.20,
  alpha2= (1-alpha1), upper_quant = FALSE, lower_quant = FALSE, upper_tot = FALSE,
  .svy = current_svy(), ...
) {
  ...
  out <- survey::svyby(
    ~`__SRVYR_TEMP_VAR__`, grps_formula, convey::svyqsr, alpha1 = alpha1, alpha2 = alpha2, upper_quant = FALSE,
    lower_quant = FALSE, upper_tot = FALSE,  na.rm = na.rm, design = .svy
  )
  out <- srvyr::get_var_est(out, vartype, grps = group_vars(.svy))
  out
...
## Ungrouped
# Calculate ungrouped for survey package
convey::svyqsr(~eqincome, des_eusilc, upper_tot = TRUE, lower_tot=TRUE )

# With our new function


survey_qsr.tbl_svy(srvyr_eusilc$variables$eqincome, upper_tot = TRUE, lower_tot=TRUE, .svy = srvyr_eusilc)


# And finally, the more typical way through summarize
srvyr_eusilc %>%  summarize(eqincome = survey_qsr(eqincome, upper_tot = TRUE, lower_tot=TRUE ))

## Groups
# Calculate by groups for survey

survey::svyby(~eqincome, ~rb090, des_eusilc, convey::svyqsr, upper_tot = TRUE, lower_tot=TRUE)

# And finally, the more typical way through summarize
srvyr_eusilc %>%
  group_by(rb090) %>%
  summarize(eqincome = survey_qsr(eqincome, upper_tot = TRUE, lower_tot=TRUE))
